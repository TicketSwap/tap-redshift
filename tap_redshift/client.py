...
class RedshiftStream(SQLStream):
    ...
    def _get_records_via_s3_unload(self) -> t.Iterable[dict[str, t.Any]]:
        """Get records by unloading to S3 as CSV first, then downloading and reading.
    ...
    def unload_to_s3(self, query: str, s3_path: str, unload_options: dict | None = None) -> str:
        """Unload query results to S3 using Redshift UNLOAD command in CSV format.
        ...
        # Default unload options for CSV format
        options = ["FORMAT CSV", "ALLOWOVERWRITE", "GZIP", "HEADER"]
    ...
    def _read_unloaded_file(self, file_path: str) -> t.Iterable[dict[str, t.Any]]:
        """Read records from an unloaded CSV file efficiently.

        Args:
            file_path: Path to the unloaded CSV file (may be gzipped)

        Yields:
            One dict per record.
        """
        self.logger.info("Reading CSV file %s", file_path)
        records = pd.read_csv(file_path, encoding="utf-8", compression="gzip", low_memory=False, header=0)
        yield from records.to_dict(orient="records")
